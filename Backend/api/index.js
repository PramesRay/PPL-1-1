...
app.post('/login', async (req, res) => {
  ...
  database.query('SELECT * FROM user WHERE username = ?', [username], async (err, result) => {
    ...
    if (!user.is_confirmed) {
      return res.status(400).json({error: true, message:'Silakan verifikasi email Anda terlebih dahulu' })
    }
...
app.get('/logout', (req, res) => {
  try {
    ...
    return res.status(200).json({error:false, message: 'Anda berhasil logout', loginStatus: false, cookiesession: req.session })
  } catch (err) {
    ...
    database.query('SELECT * FROM user WHERE email = ?', [email], async (err, res1) => {
      ...
      [email, username, hashedPassword, phone, role, token, false], async (err, res3) => {
        ...
        try {
          ...
          return res.status(200).json({error:false, message:'Registrasi berhasil. Silakan cek email Anda untuk verifikasi.'})
        } catch (error) {
          console.error('Error sending email:', error)
          return res.status(500).json({error:true, message:'Gagal mengirim email verifikasi'})
        }
        // // res.redirect('/')
...
app.get('/verify/:token', (req, res) => {
  ...
  database.query('UPDATE user SET is_confirmed = true WHERE confirmation_token = ?', [token], (err, result) => {
    ...
    return res.status(200).json({error: false, message:'Email berhasil diverifikasi. Anda sekarang dapat login.'})
  })
})
...
app.post('/post/review', async (req, res) => {
  ...
  if (!deskripsi_review || !rating || !pengguna_id) {
    return res.status(400).json({error: true, message:`Data dari ${pengguna_id} tidak seharusnya kosong` })
  }
  ...
  database.query('INSERT INTO review (deskripsi_review, rating, pengguna_id) VALUES (?, ?, ?)',
    [deskripsi_review, rating, pengguna_id], (err, results) => {
      ...
      return res.status(201).json({error: false, review_id: results.insertId, message: 'Review berhasil ditambahkan' })
    })
})

// Endpoint untuk mendapatkan semua ulasan yang diurutkan berdasarkan review_id secara terbaru
app.get('/get/review', (req, res) => {
  database.query('SELECT user.username, review.deskripsi_review, review.rating FROM review JOIN user ON review.pengguna_id = user.pengguna_id ORDER BY review.review_id DESC', (err, rows) => {
    if (err) {
      console.error('Database query error:', err)
    ...
    database.query('SELECT * FROM user WHERE email = ?', [email], async (err, res1) => {
      ...
      database.query('SELECT * FROM user WHERE nomor_telepon = ?', [nomor_telepon], async (err, res2) => {
        ...
        if (res2.length > 0 && res2[0].pengguna_id != pengguna_id) {
          return res.status(500).json({error: true, message:'Nomor Handphone sudah terdaftar' })
        }
        ...
        database.query('SELECT * FROM user WHERE username = ?', [username], async (err, res3) => {
          ...
          database.query('SELECT * FROM user WHERE pengguna_id = ?', [pengguna_id], async (err, res4) => {
            ...
            database.query('UPDATE user SET nama = ?, email = ?, username = ?, nomor_telepon = ?, alamat = ?, foto_profile = ?, jenis_kelamin = ? WHERE pengguna_id = ?', [ nama, email, username, nomor_telepon, alamat, profile, jk, pengguna_id ], (err, row) => {
              if (err) {
                console.error('Database query error:', err)
              ...
              return res.status(200).json({error: false, message: 'Profile updated successfully', data: row })
            })
          })
        })
      })
...
app.post('/check-in/', (req, res) => {
  ...
  database.query(query, [waktu_checkin, pengguna_id], function (err) {
    if (err) {
      return res.status(500).json({error: true, message: 'Failed to check in', error: err.message })
    }
    return res.status(200).json({error: false, message: 'Checked in successfully', check_in_out_id: this.lastID, check_in_status: true })
  })
})
...
app.post('/check-out/', (req, res) => {
  ...
  database.query(query, [waktu_checkout, pengguna_id], function (err) {
    if (err) {
      return res.status(500).json({error: true, message: 'Failed to check out', error: err.message })
    }
    if (this.changes === 0) {
      return res.status(400).json({error: true, message: 'No active check-in found for this user' })
    }
    return res.status(200).json({error: false, message: 'Checked out successfully', check_in_status: false })
  })
})
...
app.get('/visitor-count/', (req, res) => {
  ...
  database.query(query, [], (err, rows) => {
    if (err) {
      console.error('Database query error:', err)
      return res.status(500).json({error: true, message: 'Failed to retrieve visitor count', error: err.message })
    }
...
app.post('/transactions', (req, res) => {
  ...
  database.query("SELECT * from level where level_id = ?", [level_id], (err, result) => {
    ...
    if (!tanggal_transaksi || !total_pembayaran || !metode_pembayaran || !pengguna_id || !level_id) {
      return res.status(400).json({error: true, total_pembayaran : total_pembayaran, tanggal_transaksi : tanggal_transaksi})
    }
    ...
    database.query(query, values, (err, results) => {
      ...
      database.query(`SELECT transaksi_id FROM transaction WHERE pengguna_id = ? AND status = 'pending' ORDER BY transaksi_id DESC`, [pengguna_id], (err, results) =>  {
        if (err) throw err
        res.status(201).json({error: false, message: 'Transaction created successfully', transaksi_id:result[0]})
      })
    }) 
  })
}) 
    ...
    database.query("SELECT * FROM transaction WHERE transaksi_id = ? AND pengguna_id = ?", [transaksi_id, pengguna_id], async (err, result) => {
      ...
      if (transaction.status_pembayaran !== "pending") {
        ...
        if (req.session.transactionToken) {
          ...
          return res.status(404).json({error: true, message: 'Token transaksi tidak ditemukan.' })
        }
      } else {
        const parameter = {
          "transaction_details": {
...
app.put('/transaction-done/:id', (req, res) => {
  ...
  database.query(`UPDATE transaction SET status_pembayaran = "berhasil" WHERE transaksi_id = ?`, [transaksi_id], (err, result) => {
    ...
    database.query('UPDATE user SET role = "member" WHERE pengguna_id =?', [pengguna_id], (err, result2) => {
      ...
      return res.status(200).json({error: false, message:'Subscribe berhasil! Selamat anda sekarang adalah seorang member'})
    })
  })
})
