...
const corsConfig = {
  origin: 'https://fitivities.vercel.app', //ganti ketika ingin digunakan oleh FE
...
app.get('/', (req, res) => {
  // res.render('payment');
  const session = (req.session.loggedIn ? true : false)
  
  return res.status(200).json({ error: false, message: 'Berhasil masuk ke halaman utama', session: req.session });
});
...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    database.query('SELECT * FROM user WHERE email = ?', [email], async (err, emailRes) => {
      if (err) {
        console.error('Database query error:', err);
        return res.status(500).json({ error: true, message: 'Terjadi kesalahan pada server' });
      }
      ...
      database.query('SELECT * FROM user WHERE nomor_telepon = ?', [phone], async (err, phoneNumRes) => {
        if (err) {
          console.error('Database query error:', err);
          return res.json({ error: true, message: 'Terjadi kesalahan pada server' });
        }

        if (phoneNumRes.length > 0) {
      ...
      database.query(
        ...
        [email, username, hashedPassword, phone, role, token, 0],
        async (err, insertRes) => {
          if (err) {
          ...
          try {
            ...
            database.query(
              'SELECT role, token, is_confirmed FROM user WHERE pengguna_id = ?',
              [insertRes.insertId],
              (err, userVerifRes) => {
                if (err) {
                  console.error('Database insert error:', err);
                ...
                req.session.loggedIn = (userVerifRes[0].is_confirmed == 1) ? true : false;
                req.session.username = username;
                req.session.userId = insertRes.insertId;
                req.session.role = userVerifRes[0].role;
                req.session.token = userVerifRes[0].token
              }
            );
            return res.status(200).json({
              error: false,
            ...
            console.error('Error dalam mengirim email:', error);
            return res
              .status(500)
...
app.get('/verify/:token', (req, res) => {
  ...
  database.query(
    ...
    (err, result) => {
      ...
      return res
        .status(200)
        .json({ error: false, message: 'Email berhasil diverifikasi. Anda sekarang dapat login.', session: req.session });
    }
  );
});
...
app.put('/update/profile/', (req, res) => {
  const pengguna_id = req.session.userId;
  var { nama, email, username, nomor_telepon, alamat, profile, jk } = req.body;
  ...
  if (pengguna_id) {
    ...
    database.query('SELECT * FROM user WHERE pengguna_id = ?', [pengguna_id], async (err, res0) => {
      if (err) {
        console.error('Database query error:', err);
        return res.status(500).json({ error: true, message: 'Terjadi kesalahan pada server' });
      }
      ...
      if (nomor_telepon == '') {
        nomor_telepon = res0[0].nomor_telepon;
      }
      if (alamat == '') {
        alamat = res0[0].alamat;
      }
      if (profile == '') {
        profile = res0[0].profile;
      }
      if (jk == '') {
        jk = res0[0].jk;
      }
...
app.get('/transactions', (req, res) => {
  res.render("transaction/subscribe")
})
...
app.post('/transactions/', (req, res) => {
  ...
  database.query('SELECT * from level where level_id = ?', [level_id], (err, levelRes) => {
    if (err) throw err;
    const total_pembayaran = levelRes[0].harga;
    ...
    database.query(query, values, (err, inputTransactionRes) => {
      if (err) throw err;

      database.query(
        `SELECT transaksi_id FROM transaction WHERE pengguna_id = ? AND status_pembayaran = 'pending' ORDER BY transaksi_id DESC`,
        [pengguna_id],
        (err, transactionIdRes) => {
          if (err) throw err;
          req.session.transactionId = transactionIdRes[0].transaksi_id
          return res.status(201).json({
            error: false,
            message: 'Transaction created successfully',
            transaksi_id: transactionIdRes[0].transaksi_id,
          });
        }
      );
    });
...
app.get('/get-transaction-token/', async (req, res) => {
  // const pengguna_id = req.params.userId;
  const pengguna_id = req.session.userId;
  // const transaksi_id = req.params.id;
  const transaksi_id = req.session.transactionId;
  ...
  try {
    database.query(
      ...
      async (err, result) => {
        ...
        if (result.length === 0) {
          console.log(transaksi_id, pengguna_id);
...
app.put('/transaction-done/', (req, res) => {
  // const pengguna_id = req.params.userId;
  // const transaksi_id = req.params.id;
  const pengguna_id = req.session.userId;
  const transaksi_id = req.session.transactionId;
...
app.delete('/transaction-failed/', (req, res) => {
  // const pengguna_id = req.session.userId;
  // const transaksi_id = req.params.id;
  const pengguna_id = req.session.userId;
  const transaksi_id = req.session.transactionId;
